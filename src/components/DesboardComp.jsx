...
const DesboardComp = () => {
  ...
  const operationalStats = [
    { id: 1, label: "Today's Sales", value: "৳15,460", icon: <FiHome size={22} className="text-blue-400" /> },
    { id: 2, label: "Followup", value: "৳1,070", icon: <FiUsers size={22} className="text-cyan-400" /> },
    { id: 3, label: "Confirm + Shipped", value: "৳586,840", icon: <FiTruck size={22} className="text-sky-400" /> },
    { id: 4, label: "Delivered", value: "80", icon: <FiCheckCircle size={22} className="text-emerald-400" /> },
    { id: 5, label: "Cancelled", value: "৳51,930", icon: <FiXCircle size={22} className="text-rose-500" /> },
    { id: 6, label: "Returned + Fail", value: "৳4,320", icon: <FiCornerUpLeft size={22} className="text-amber-400" /> },
  ];
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto justify-end">
          ...
          <div className="bg-[#081730]/70 backdrop-blur-md border border-slate-800/90 p-2.5 px-5 rounded-xl flex items-center gap-3 shadow-lg select-none">
            <div className="text-left">
              <p className="text-xs font-black text-slate-200 tracking-wide font-mono uppercase">
                May 12, 2025
              </p>
              <p className="text-[11px] text-slate-500 font-bold tracking-wider uppercase mt-0.5">Monday</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="bg-[#081730]/40 backdrop-blur-md p-6 rounded-xl border border-slate-800/80 shadow-xl flex items-center justify-between relative overflow-hidden min-h-[115px] group hover:border-slate-700 transition-all">
            <div className="space-y-2 z-10">
              <div className="flex items-center gap-2.5">
                ...
                <span className="bg-blue-500/10 text-blue-400 text-[10px] px-2 py-0.5 rounded border border-blue-500/10 font-mono">5/12/2025</span>
              </div>
              <p className="text-3xl font-black text-white tracking-tight font-mono">3</p>
            </div>
          ...
          {overviewStats.map((stat) => (
            <div key={stat.id} className="bg-[#081730]/40 backdrop-blur-md p-6 rounded-xl border border-slate-800/80 shadow-xl flex flex-row items-center justify-between min-h-[115px] relative overflow-hidden group hover:border-slate-700 transition-all duration-300">
              <div className="flex items-center gap-4 z-10">
                <div className={`p-3.5 text-white rounded-xl ${stat.iconBg}`}>
                  {stat.icon}
                </div>
                <div className="space-y-0.5">
                  <p className={`text-2xl font-black font-mono tracking-tight ${stat.textColor || 'text-white'}`}>
                    {stat.count}
                  </p>
